...
-(id)initWithStrUrl:(NSString*)strUrl Time:(NSInteger)time Block:(void(^)(BOOL,HttpDownLoadBlock*))a{
    if (self=[super init]) {
        ...
        if ([manager fileExistsAtPath:self.path]&&![manager timeOutWithPath:[MyMD5 md5:strUrl] timeOut:time]) {
...
-(void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error
{
    ...
    UIAlertView*alertView=[[UIAlertView alloc]initWithTitle:@"提示" message:@"您的网络有问题，请检查网络" delegate:nil cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
    [alertView show];
...
-(void)jsonValue{
    ...
    if(self.data) {
        ...
        if ([result isKindOfClass:[NSArray class]]) {
            self.dataArray = result;
            NSLog(@"打印NSArray:%@",result);
        }else if([result isKindOfClass:[NSDictionary class]]){
            self.dataDic = result;
            NSLog(@"打印NSDictionary%@",result);
        }
        else
        {
            id result=[[NSString alloc] initWithData:self.data encoding:NSUTF8StringEncoding];
            
            if([result isKindOfClass:[NSString class]])
            {
                self.dataStr=result;
                NSLog(@"打印NSString:%@",self.dataStr);
            }
            else
            {
                self.dataImage=[UIImage imageWithData:self.data];
                NSLog(@"打印UIImage%@",[[NSString alloc] initWithData:self.data encoding:NSUTF8StringEncoding]);
            }
